...
def export_middle_tweeze_config_to_julia(
    omega_tweezer,
    linewidths,
    omega_res,
    m,
    ...
    """
    Runs tweezer_combos_full_radial_middle_only for a single (N, P) configuration
    and writes the resulting mode frequencies/eigenvectors/tweezed-ion indices out
    as a small Julia source file, ready to `include(...)` from an IonSim notebook
    (e.g. test.ipynb) -- this replaces hand-copying numbers out of a DataFrame.

    Inputs:
    omega_tweezer, linewidths, omega_res, m, mode_calc_r, qubit_lambda
        -- same as tweezer_combos_full_radial_middle_only
    N : int
        number of ions (single value, not a list -- this exports exactly one config)
    f_rf_r, f_rf_a : float
        radial/axial rf trapping frequencies [Hz]
    P : float
        tweezer power [W] (single value, not a sweep)
    w0 : float
        tweezer beam waist [m]
    filepath : str
        where to write the generated .jl file

    Returns:
    the one-row DataFrame (same as tweezer_combos_full_radial_middle_only), in case
    you want to inspect it before/after trusting the exported file.
    """
    ueq_dict = {N: ion_spacing(N, f_rf_a * 2 * np.pi)[0]}
    result_df = tweezer_combos_full_radial_middle_only(
        omega_tweezer, linewidths, omega_res, m, mode_calc_r,
        [N], f_rf_r, ueq_dict, P, w0, max_tweezed=1, qubit_lambda=qubit_lambda,
    )
    row = result_df.iloc[0]

    freqs = [float(row[f"Mode{i}_freq"]) for i in range(N)]
    vecs = [np.ravel(row[f"Mode{i}_eigvec"]).astype(float).tolist() for i in range(N)]
    # IonSim/Julia ions are 1-indexed; the DataFrame's "Tweezed ions" are 0-indexed
    tweezed_1based = [int(i) + 1 for i in row["Tweezed ions"]]
    ...
    lines = [
        "# Auto-generated by export_middle_tweeze_config_to_julia (tweezer_functions.py).",
        "# Do not edit by hand -- rerun that function to regenerate.",
        f"N_pipeline = {N}",
        f"tweezed_ions_pipeline = {jl_int_vec(tweezed_1based)}  # 1-based (IonSim convention)",
        f"P_pipeline = {P!r}  # tweezer power, W",
        f"mode_freqs_pipeline = {jl_float_vec(freqs)}",
        "mode_vecs_pipeline = [",
    ]
    for v in vecs:
        lines.append(f"    {jl_float_vec(v)},")
    lines.append("]")
...
def export_middle_tweeze_config_sweep_to_julia(
    omega_tweezer,
    linewidths,
    omega_res,
    m,
    ...
    """
    Same idea as export_middle_tweeze_config_to_julia, but for a LIST of
    tweezer powers P_sweep instead of a single one. Writes ONE .jl file
    containing, for every power in the sweep, that power's mode
    frequencies/eigenvectors -- so a Julia-side sweep (e.g. looking for the
    power where a phonon-number exchange fully swaps to ~0) can loop over
    them without needing one file per power.

    Inputs: same as export_middle_tweeze_config_to_julia, except
    P_sweep : iterable of float
        tweezer powers [W] to scan (was a single P before)

    Returns:
    list of the one-row DataFrames (same as tweezer_combos_full_radial_middle_only),
    one per power in P_sweep, in case you want to inspect them.
    """
    ueq_dict = {N: ion_spacing(N, f_rf_a * 2 * np.pi)[0]}
    ...
    for P in P_sweep:
        ...
        row = result_df.iloc[0]
        all_freqs.append([float(row[f"Mode{i}_freq"]) for i in range(N)])
        all_vecs.append([np.ravel(row[f"Mode{i}_eigvec"]).astype(float).tolist() for i in range(N)])
        if tweezed_1based is None:
            # tweezed ion(s) don't depend on power -- only computed once
            tweezed_1based = [int(i) + 1 for i in row["Tweezed ions"]]

        # tweezer trap frequency [Hz] this power corresponds to, so the
        # sweep results can be read/plotted against a physical frequency
        # instead of just raw power
        U = potential(omega_tweezer, linewidths, omega_res, P, w0)
        omega_tw_r = omega_tweezer_r(U, w0, m)
        f_tweezer_sweep.append(float(omega_tw_r / (2 * np.pi)))
    ...
    lines = [
        "# Auto-generated by export_middle_tweeze_config_sweep_to_julia (tweezer_functions.py).",
        "# Do not edit by hand -- rerun that function to regenerate.",
        f"N_pipeline = {N}",
        f"tweezed_ions_pipeline = {jl_int_vec(tweezed_1based)}  # 1-based (IonSim convention)",
        f"P_sweep_pipeline = {jl_float_vec(P_sweep)}  # W",
        f"f_tweezer_sweep_pipeline = {jl_float_vec(f_tweezer_sweep)}  # tweezer trap freq per P_sweep_pipeline entry, Hz (= omega_tweezer_r/(2*pi))",
        "mode_freqs_sweep_pipeline = [  # one entry per P_sweep_pipeline",
    ]
    for freqs in all_freqs:
        lines.append(f"    {jl_float_vec(freqs)},")
    lines.append("]")
    lines.append("mode_vecs_sweep_pipeline = [  # one entry per P_sweep_pipeline")
